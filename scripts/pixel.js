...
POWDER: function(pixel) {
    if (pixel.drag) { doDefaults(pixel); return; }
    ...
    var startX = pixel.x;
    var startY = pixel.y;
    pixelMap[startX][startY] = undefined; // Move out of the way for collision checks

    var newX = startX + pixel.vx;
    var newY = startY + pixel.vy;
    ...
    for (var i = 1; i <= steps; i++) {
        var checkX = Math.round(startX + stepX*i);
        var checkY = Math.round(startY + stepY*i);
        ...
        if (!isEmpty(checkX, checkY)) {
            pixel.x = lastX;
            pixel.y = lastY;
            pixelMap[lastX][lastY] = pixel;

            if (!outOfBounds(checkX, checkY)) {
                var otherPixel = pixelMap[checkX][checkY];
                if (otherPixel && elements[otherPixel.element].movable) {
                    var tempVx = pixel.vx;
                    var tempVy = pixel.vy;
                    var otherVx = otherPixel.vx || 0;
                    var otherVy = otherPixel.vy || 0;

                    var density1 = elements[pixel.element].density || 1;
                    var density2 = elements[otherPixel.element].density || 1;
                    var totalDensity = density1 + density2;

                    var newVx1 = ((density1 - density2) * (pixel.vx||0) + 2 * density2 * otherVx) / totalDensity;
                    var newVy1 = ((density1 - density2) * (pixel.vy||0) + 2 * density2 * otherVy) / totalDensity;
                    var newVx2 = (2 * density1 * (pixel.vx||0) + (density2 - density1) * otherVx) / totalDensity;
                    var newVy2 = (2 * density1 * (pixel.vy||0) + (density2 - density1) * otherVy) / totalDensity;

                    pixel.vx = newVx1 * 0.8;
                    pixel.vy = newVy1 * 0.8;
                    otherPixel.vx = newVx2 * 0.8;
                    otherPixel.vy = newVy2 * 0.8;
                }
            }
            // Simple bounce
...
LIQUID: function(pixel) {
    ...
    // Gravity
    pixel.vy += 0.2; // Heavier than powder

    // Viscosity & Air Resistance
    var viscosity = elements[pixel.element].viscosity || 1;
    pixel.vx *= (1 - (0.01 * viscosity));
    pixel.vy *= (1 - (0.01 * viscosity));

    var startX = pixel.x;
    var startY = pixel.y;
    pixelMap[startX][startY] = undefined;
        ...
        if (!isEmpty(checkX, checkY)) {
            ...
            if (!outOfBounds(checkX, checkY)) {
                ...
                if (otherPixel && elements[otherPixel.element].movable) {
                    ...
                    var newVx1 = ((density1 - density2) * (pixel.vx||0) + 2 * density2 * (otherPixel.vx||0)) / totalDensity;
                    var newVy1 = ((density1 - density2) * (pixel.vy||0) + 2 * density2 * (otherPixel.vy||0)) / totalDensity;
                    var newVx2 = (2 * density1 * (pixel.vx||0) + (density2 - density1) * (otherPixel.vx||0)) / totalDensity;
                    var newVy2 = (2 * density1 * (pixel.vy||0) + (density2 - density1) * (otherPixel.vy||0)) / totalDensity;

                    pixel.vx = newVx1 * 0.9;
                    pixel.vy = newVy1 * 0.9;
                    otherPixel.vx = newVx2 * 0.9;
                    otherPixel.vy = newVy2 * 0.9;
                }
            }
            if (Math.abs(dx) > Math.abs(dy)) {
                pixel.vx *= -0.3;
        ...
        if (!isEmpty(checkX, checkY)) {
            ...
            if (!outOfBounds(checkX, checkY)) {
                ...
                if (otherPixel && elements[otherPixel.element].movable) {
                    ...
                    pixel.vx = newVx1 * 0.95;
                    pixel.vy = newVy1 * 0.95;
                    otherPixel.vx = newVx2 * 0.95;
                    otherPixel.vy = newVy2 * 0.95;
                }
            }
            if (Math.abs(dx) > Math.abs(dy)) {
                pixel.vx *= -0.9;
